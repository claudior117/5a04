...
Begin VB.Form prod_verpedidos 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox c_estado 
         Height          =   315
         ItemData        =   "pro004.frx":030A
         Left            =   1200
         List            =   "pro004.frx":030C
         TabIndex        =   17
         Top             =   240
         Width           =   2415
      End
   ...
   End
End
Attribute VB_Name = "prod_verpedidos"
...
Sub carga_oc()
    Call busca
     r = 1
     While Not rs.EOF
       If rs("num_int_oc") = 0 Then
         noc = "0000-00000000"
       Else
         Set rs1 = New ADODB.Recordset
         q = "select * from a5 where [num_int] = " & rs("num_int_oc") & " and [id_tipocomp] = 65"
         rs1.Open q, cn1
         If Not rs1.EOF And Not rs1.BOF Then
            noc = Format$(rs1("sucursal"), "0000") & "-" & Format$(rs1("num_comprobante"), "00000000")
         Else
            noc = "0000-00000000"
         End If
         Set rs1 = Nothing
       End If
        i = rs("num_referencia")
        msf1.AddItem r & Chr(9) & i & Chr(9) & Format$(rs("id_producto"), "00000") & Chr(9) & rs("pro_02.descripcion") & Chr(9) & Format$(rs("cantidad"), "######0.00") & Chr(9) & noc & Chr(9) & Format$(rs("cantidad_oc"), "######0.00") & Chr(9) & Format$(rs("num_comprobante"), "00000000") & Chr(9) & rs("pro_02.observaciones") & Chr(9) & rs("fecha") & Chr(9) & rs("pro_02.fecha_esperado") & Chr(9) & rs("pro_01.id_usuario") & Chr(9) & rs("usuario") & Chr(9) & rs("pro_02.estado") & Chr(9) & rs("a4.descripcion")
        rs.MoveNext
        r = r + 1
     Wend
     'Set rs = Nothing
...
Sub busca()
     ...
     q = "select * from pro_01, pro_02, g1, a4 where  pro_01.[num_int] = pro_02.[num_int] and pro_01.[id_usuario] = g1.[id_usuario] and pro_01.[id_obra] = a4.[id_obra] "
     
     If c_prov.ListIndex > 0 Then
         q = q & " and pro_01.[id_obra] = " & c_prov.ItemData(c_prov.ListIndex)
     End If
     
    If c_estado.ListIndex > 0 Then
         q = q & " and pro_02.[estado] = '" & Mid$(c_estado, 2, 1) & "'"
    End If

    If t_fecha1 <> "" Then
      If IsDate(t_fecha1) Then
        q = q & " and datevalue([fecha]) >= datevalue('" & t_fecha1 & "') "
      End If
    End If

    If t_fecha2 <> "" Then
     If IsDate(t_fecha2) Then
      q = q & " and datevalue(pro_02.[fecha_esperado]) >= datevalue('" & t_fecha2 & "') "
     End If
    End If
    ...
    Select Case c_prod.ListIndex
     ...
     Case Is < 0
     If c_prod <> "" Then
       q = q & " and pro_02.[descripcion] like '%" & c_prod & "%'"
       c = " and "
     End If
    End Select
    ...
    If c_usuario.ListIndex > 0 Then
         q = q & " and pro_01.[id_usuario] = " & c_usuario.ListIndex
    End If
     
         
    If Option1 = True Then
       q = q & " order by pro_02.[num_referencia]"
    Else
      If Option2 = True Then
         q = q & " order by pro_02.[fecha_esperado]"
      Else
         q = q & " order by pro_01.[id_obra]"
      End If
    End If
...
msf1.ColWidth(9) = 1200
msf1.ColWidth(10) = 1200
msf1.ColWidth(11) = 400
...
msf1.TextMatrix(0, 4) = "Cant.Sol"
msf1.TextMatrix(0, 5) = "Nro. OC."
msf1.TextMatrix(0, 6) = "Cant.OC"
msf1.TextMatrix(0, 7) = "Nro. Sol."
msf1.TextMatrix(0, 8) = "Obs."
msf1.TextMatrix(0, 9) = "Fecha Ped."
msf1.TextMatrix(0, 10) = "Fecha Sol."
msf1.TextMatrix(0, 11) = "Id."
msf1.TextMatrix(0, 12) = "Usuario"
msf1.TextMatrix(0, 13) = "Estado"
msf1.TextMatrix(0, 14) = "Obra"
...
c_estado.AddItem "[T] <Todos>", 0
c_estado.AddItem "[P] Pendientes(sin OC)", 1
c_estado.AddItem "[O] En O.C", 2
