...
Begin VB.Form vta_retenciones 
   ...
   Begin VB.Frame Frame2 
      ...
      Top             =   2040
      Width           =   9375
      Begin VB.TextBox t_certificado 
         BorderStyle     =   0  'None
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   12
      ...
      Begin VB.Label Label7 
         ...
         BackColor       =   &H00800080&
         Caption         =   "Cod. operacion o Nro. certificado"
         ForeColor       =   &H00FFFFFF&
         Height          =   375
         Left            =   120
         TabIndex        =   51
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox c_sucursal 
         Height          =   315
         ItemData        =   "vta039.frx":0105
         Left            =   7440
         List            =   "vta039.frx":0107
      ...
      Begin VB.ComboBox c_tipocomp 
         Height          =   315
         ItemData        =   "vta039.frx":0580
         Left            =   1680
         List            =   "vta039.frx":0582
         Style           =   2  'Dropdown List
         TabIndex        =   0
         Top             =   240
         Width           =   3135
   ...
   End
End
Attribute VB_Name = "vta_retenciones"
...
Sub carga()
  ...
  If Not rs.BOF And Not rs.EOF Then
     ...
     t_obs = rs("observaciones")
     c_regimen.ListIndex = buscaindice(c_regimen, rs("total_bultos"))
     t_certificado = rs("valor_declarado")
     t_cotizacion = Format$(rs("cotizacion_dolar"), "#####0.000")
     T_total2 = Format$(rs("total_otra_moneda"), "######0.00")
...
Else
  ...
  If c_prov.ListIndex > 0 Then
    Call iniciagraba
  Else
    MsgBox ("El Cliente Manual solo puede utilizarse para contado")
  End If
End If
...
Private Sub c_regimen_LostFocus()
If c_regimen.ListIndex < 0 Then
  c_regimen.ListIndex = 0
End If
End Sub
...
Case Is = 100 'ret ib
  Call carga_regimen_retvta(c_regimen, 1)
Case Is = 101 'ret iva
  Call carga_regimen_retvta(c_regimen, 767)
Case Is = 102 'ret ganancia
  Call carga_regimen_retvta(c_regimen, 217)
Case Is = 103 'ret suss
  Call carga_regimen_retvta(c_regimen, 736)
Case Else
  c_regimen.clear
  c_regimen.AddItem "<Sin identificar", 0
End Select
...
c_sucursal.ListIndex = buscaindice(c_sucursal, glo.sucursal)
Set rs = New ADODB.Recordset
q = "select * from vta_06 where [sucursal] = " & glo.sucursal & " and  [id_tipocomp] >= 100 and  [id_tipocomp] <= 105 order by descripcion"
rs.Open q, cn1
Call llena_combo(rs, "descripcion", "id_tipocomp", c_tipocomp, True)
Set rs = Nothing

c_tipocomp.ListIndex = buscaindice(c_tipocomp, 100)
...
'en total_bultos guardo el codigo de regimen
'en valor-declarado guardo el numero de certificado
...
", 0, " & Val(t_perciva) & ", " & codact & ", " & Val(t_alicuotaib) & ", " & Val(t_alicuotaperciva) & ", " & Check1 & ", '" & t_fechavto & "', " & c_regimen.ItemData(c_regimen.ListIndex) & _
", " & Val(t_certificado) & ", ' ', ' ', ' ', 0, " & Val(c_sucursal) & _
", '" & Left$(vta_clientes.t_cli, 50) & "', '" & Left$(vta_clientes.t_direccion, 50) & "', '" & Left$(vta_clientes.t_cuit, 20) & "', '" & Left$(vta_clientes.t_localidad, 50) & "', " & tiporespiva & _
", ' ', ' ', ' ', " & ssi & ", " & para.z_actual & ", 'u2', '01/01/2018', 1)"

'MsgBox (QUERY)
    cn1.Execute QUERY
      
     
'agrego renglon en vta_03
...
'QUERY = QUERY & " VALUES (" & numint & ", 1, 1, '" & c_tipocomp & " ', 1, " & Val(t_total) & ", " & Val(t_total) & ", 0, 0, 0, 1, 'U'," & Val(t_total) & ", 0)"
'cn1.Execute QUERY
   ...
   If Generaasientosauto Then
     If cl_compvta.contabilidad <> "N" Then
         ...
         Select Case c_tipocomp.ItemData(c_tipocomp.ListIndex)
         Case Is = 100
           cuentaact = para.cuenta_retibbav
           dcta = "Ret. IB"
         Case Is = 101
             cuentaact = para.cuenta_retivav
             dcta = "Ret. Iva"
         Case Is = 102
             cuentaact = para.cuenta_retganv
             dcta = "Ret. Gan."
         Case Is = 103
             cuentaact = para.cuenta_retsussv
             dcta = "Ret. Suss"
         End Select
         ...
         QUERY = "INSERT INTO c_03([num_interno], [renglon], [id_cuenta], [ubicacion], [importe], [descripcion])"
         QUERY = QUERY & " VALUES (" & numintcgr & ", " & ic & ", " & cuentaact & ", '" & u2 & "', " & Format(importe, "######0.00") & ", '" & dcta & "')"
         cn1.Execute QUERY
         ic = ic + 1
     ...
     QUERY = QUERY & " VALUES ('Emitir retencion NI:" & numint & "', " & para.id_usuario & ", 'V', " & numint & ", '" & Now & "', '[" & c_tipocomp.ItemData(c_tipocomp.ListIndex) & "] " & t_letra & " " & Format$(Val(t_sucursal), "0000") & "-" & Format$(Val(t_numcomp), "00000000") & "', 12, " & idcli & ")"
...
Private Sub t_certificado_LostFocus()
If t_certificado = "" Then
   Select Case c_regimen.ItemData(c_regimen.ListIndex)
    Case Is = 785, Is = 786, Is = 787, Is = 794, Is = 795, Is = 796
      MsgBox ("Para este regimen el numero e certificado no puede estar vacio")
      t_certificado = "000000000001"
    Case Else
      t_certificado = "000000000000"
   End Select
End If
