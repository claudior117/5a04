...
' Mensajes / Constantes para usar con SendMessage
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Public Sub verifica_fechacorte(ByVal F As TextBox)
'devuelve vrdadera si la fecha ingresada es correcta es decir posterior a la fecha de corte
If DateValue(F) < DateValue(para.fechacorte) Then
  MsgBox ("Fecha No permitida. La fecha ingresada es anterior a la fecha de corte permitida en el sistema(ver Parametros Generales)")
...
End Sub
Sub carga_percepciones(c As ComboBox, tipo As String)
'tipo P percepciones  // R retecniones // Todas
 'marcas
q = "select * from a12"
tipo = Format$(tipo, ">@")
Set rs = New adodb.Recordset
...
While Not rs.EOF
  c.AddItem rs("tasaib")
'FIXIT: c.ItemData(c.NewIndex property no tiene equivalente en Visual Basic .NET y no se actualizará.     FixIT90210ae-R7593-R67265
  c.ItemData(c.NewIndex) = rs("id_tasaib")
  rs.MoveNext
Wend
...
If s = 0 Then
  c.AddItem "Sin Clientes y Proveedores asociados"
End If
  c.ListIndex = 0
Set rs = Nothing
End Sub
...
While Not rs.EOF
  ...
  rs.MoveNext
Wend
c.ListIndex = buscaindice(c, para.punto_venta_usuario)
Set rs = Nothing
End Sub
...
Function pone_guiones_cuit(ByVal nc) As String
'recibe el cuit numerico y le pone guines
c = Format$(nc, "00000000000")
CUIT = Mid$(c, 1, 2) & "-" & Mid$(c, 3, 8) & "-" & Mid$(c, 11, 1)
pone_guiones_cuit = CUIT
End Function
