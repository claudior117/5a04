...
Begin VB.Form vta_fact_viaje 
   BackColor       =   &H00E0E0E0&
   Caption         =   "FACTURACION DE FLETES"
   ClientHeight    =   8490
   ClientLeft      =   3180
   ClientTop       =   1380
   ClientWidth     =   11880
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox c_sucursal 
         Height          =   315
         ItemData        =   "vta036A.frx":0105
         Left            =   7440
         List            =   "vta036A.frx":0107
      ...
      Begin VB.ComboBox c_tipocomp 
         Height          =   315
         ItemData        =   "vta036A.frx":0580
         Left            =   1680
         List            =   "vta036A.frx":0582
         Style           =   2  'Dropdown List
         TabIndex        =   0
         Top             =   240
         Width           =   3135
   ...
   End
End
Attribute VB_Name = "vta_fact_viaje"
...
Sub normal()
  Set rs = New ADODB.Recordset
  q = "select * from vta_02 where [sucursal] = " & Val(t_sucursal) & " and letra = '" & t_letra & "' and [id_tipocomp] = 4 and [num_comp] = " & Val(t_numcomp)
  rs.Open q, cn1
  If Not rs.BOF And Not rs.EOF Then
      EXISTE = "S"
...
msf1.TextMatrix(0, 3) = "Chofer/camion"
msf1.TextMatrix(0, 4) = "Origen"
msf1.TextMatrix(0, 5) = "Destino"
msf1.TextMatrix(0, 6) = "C.Porte"
msf1.TextMatrix(0, 7) = "Ton."
msf1.TextMatrix(0, 8) = "Kmts"
msf1.TextMatrix(0, 9) = "Tarifa"
msf1.TextMatrix(0, 10) = "% Iva"
msf1.TextMatrix(0, 11) = "Importe"
msf1.TextMatrix(0, 12) = "PU Final"
msf1.TextMatrix(0, 13) = "Iva"
msf1.TextMatrix(0, 14) = "Total Flete"
...
If t_letra = "A" Then
  ...
  For i = 1 To msf1.Rows - 1
      r = Val(msf1.TextMatrix(i, 11))
      s = s + r
      v = v + (r * Val(msf1.TextMatrix(i, 10)) / 100)
      
      'agrega en composicion de iva
      X = 1
      While X < vta_facturacion2.msf1.Rows
        If Val(vta_facturacion2.msf1.TextMatrix(X, 0)) = Val(msf1.TextMatrix(i, 10)) Then
           vta_facturacion2.msf1.TextMatrix(X, 1) = Val(vta_facturacion2.msf1.TextMatrix(X, 1)) + r
           vta_facturacion2.msf1.TextMatrix(X, 2) = Val(vta_facturacion2.msf1.TextMatrix(X, 2)) + (r * Val(msf1.TextMatrix(i, 10)) / 100)
 ...
 Else
  ...
  For i = 1 To msf1.Rows - 1
      r = Val(msf1.TextMatrix(i, 11))
      R2 = Val(msf1.TextMatrix(i, 12))
      s = s + r
      t = t + (R2 * Val(msf1.TextMatrix(i, 7)))
...
'Set rs = Nothing
c_tipocomp.AddItem "Factura de Fletes", 0
...
If KeyCode = vbKeyInsert Then
   vta_fact_viaje1.t_renglon = ""
   vta_fact_viaje1.t_cantidad = ""
   vta_fact_viaje1.t_pu = ""
   vta_fact_viaje1.t_importe = ""
   If msf1.Rows - 1 < cantlineas Then
     vta_fact_viaje1.Show
   Else
...
QUERY = QUERY & " VALUES (" & numint & ", " & Val(t_sucursal) & ", " & Val(t_numcomp) & ", '" & t_letra & "', 4, " & idcli & _
", '" & t_fecha & "', " & para.id_usuario & ", " & Val(t_subtotal) & ", " & Val(t_nograbado) & ", " & Val(t_iva) & ", " & Val(t_total) & _
...
", 0, " & Val(t_perciva) & ", " & codact & ", " & Val(t_alicuotaib) & ", " & Val(t_alicuotaperciva) & ", 0, '" & t_fechavto & "', 0, 0, ' ', ' ', ' ', 0, " & Val(c_sucursal) & _
...
"', " & tiporespiva & ", ' ', ' ', ' ', " & ssi & ", " & para.z_actual & ", 'u2', '01/01/2018', 1)"
      ...
      For i = 1 To msf1.Rows - 1
        ...
        QUERY = "INSERT INTO vta_011([num_int], [RENGLON], [fecha], [detalle], [chofer], [origen], [destino], [carta_porte], [toneladas], [kmts], [tarifa], [importe], [tasa_iva], [iva], [importe_total])"
        QUERY = QUERY & " VALUES (" & numint & ", " & Val(msf1.TextMatrix(i, 0)) & ", '" & msf1.TextMatrix(i, 1) & "', '" & Left$(msf1.TextMatrix(i, 2), 50) & " ', '" & Left$(msf1.TextMatrix(i, 3), 50) & _
         "', '" & Left$(msf1.TextMatrix(i, 4), 50) & "', '" & Left$(msf1.TextMatrix(i, 5), 50) & "', '" & Left$(msf1.TextMatrix(i, 6), 15) & "', " & Val(msf1.TextMatrix(i, 7)) & ", " & Val(msf1.TextMatrix(i, 8)) & _
         ", " & Val(msf1.TextMatrix(i, 9)) & ", " & Val(msf1.TextMatrix(i, 11)) & ", " & Val(msf1.TextMatrix(i, 10)) & ", " & Val(msf1.TextMatrix(i, 13)) & ", " & Val(msf1.TextMatrix(i, 14)) & ")"
         cn1.Execute QUERY
      ...
      If cl_compvta.grabado <> "N" Then
       If verificatasaunica Then
          ...
          QUERY = QUERY & " VALUES (" & numint & ", " & Val(msf1.TextMatrix(1, 10)) & ", " & Val(t_iva) & ", " & Val(t_subtotal) & ", " & tiporespiva & ")"
          cn1.Execute QUERY
       Else
        For i = 1 To 7
     ...
     QUERY = QUERY & " VALUES ('Emitir Factura Flete NI:" & numint & "', " & para.id_usuario & ", 'V', " & numint & ", '" & Now & "', '[" & c_tipocomp.ItemData(c_tipocomp.ListIndex) & "] " & t_letra & " " & Format$(Val(t_sucursal), "0000") & "-" & Format$(Val(t_numcomp), "00000000") & "', 12, " & idcli & ")"
