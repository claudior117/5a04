...
Begin VB.Form stk_ingreso 
   BackColor       =   &H00E0E0E0&
   Caption         =   "INGRESO DE MERCADERIA A STOCK"
   ClientHeight    =   8805
   ClientLeft      =   75
   ClientTop       =   360
   ClientWidth     =   11925
...
If KeyCode = vbKeyF9 Then
 If msf1.Rows > 1 Then
  J = MsgBox("Confirma Grabar Ingreso Stock ", 4)
  If J = 6 Then
...
If KeyCode = vbKeyInsert Then
   stk_ingreso2.t_renglon = ""
   stk_ingreso2.t_cantidad = ""
   stk_ingreso2.Show
End If
End Sub
...
If EXISTE = "N" Then
   'oc nueva
      ...
      QUERY = QUERY & " VALUES ('" & t_fecha & "', '" & t_letra & "', " & Val(t_comp) & ", " & para.id_usuario & ", '" & t_detalle & " ', " & Val(t_sucursal) & ", 20, " & c_prov.ItemData(c_prov.ListIndex) & ", 1)"
      ...
      For i = 1 To msf1.Rows - 1
        ...
        QUERY = QUERY & " VALUES (" & numint & ", " & Val(msf1.TextMatrix(i, 0)) & ", " & Val(msf1.TextMatrix(i, 1)) & ", '" & msf1.TextMatrix(i, 2) & "', ' ', '" & msf1.TextMatrix(i, 4) & "', " & msf1.TextMatrix(i, 3) & " , 'E')"
        cn1.Execute QUERY
      
        QUERY = "INSERT INTO stk_01([fecha], [id_producto], [cantidad], [ubicacion], [comprobante], [descripcion], [num_mov_int], [modulo])"
        QUERY = QUERY & " VALUES ('" & t_fecha & "', " & Val(msf1.TextMatrix(i, 1)) & ", " & msf1.TextMatrix(i, 3) & ", 'E', 'Ingreso Stk " & Format$(numint, "00000000") & "', '" & Left$(c_prov, 49) & " ', " & numint & ", 'S')"
        cn1.Execute QUERY
...
Private Sub t_comp_LostFocus()
t_comp = Format$(Val(t_comp), "00000000")

Set rs = New ADODB.Recordset
q = "select * from stk_02 where [id_proveedor] = " & c_prov.ItemData(c_prov.ListIndex) & " and [letra] = '" & t_letra & "' and [sucursal] = " & Val(t_sucursal) & " and [num_comprobante] = " & Val(t_comp)
MsgBox (q)
rs.Open q, cn1
If Not rs.EOF And Not rs.BOF Then
   MsgBox ("El comprobante ya fue ingresado. Verifiquelo")
End If
Set rs = Nothing
End Sub
