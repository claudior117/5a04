...
Begin VB.Form vta_estadocuentadual 
   BackColor       =   &H00E0E0E0&
   ...
   End
End
Attribute VB_Name = "vta_estadocuentadual"
...
Sub carga()
  ...
  QUERY = QUERY & " VALUES ('Emision estado Cuenta Dual: " & c_prov.ItemData(c_prov.ListIndex) & "-" & c_prov2.ItemData(c_prov2.ListIndex) & "', " & para.id_usuario & ", 'V', 0, '" & Now & "', '" & Left$(c_prov, 50) & "', 10, " & c_prov.ItemData(c_prov.ListIndex) & ")"
  cn1.BeginTrans
  cn1.Execute QUERY
  cn1.CommitTrans
  ...
  While Not rs.EOF
    ...
    End If
     ...
     Else
      ...
      End If
     End If
     'proforma de ventas van en 0 cuando estan canceladas
     ...
     Else
        msf1.AddItem F & Chr(9) & CTC & Chr(9) & tc & Chr(9) & nc & Chr(9) & d & Chr(9) & h & Chr(9) & s & Chr(9) & s2 & Chr(9) & o & Chr(9) & ni
     End If
    rs.MoveNext
  Wend
...
If Check1 = 0 Then
  ...
  msf2.Rows = 1
  msf2.Cols = 10
  msf2.ColWidth(0) = 1300
  msf2.ColWidth(1) = 500
  msf2.ColWidth(2) = 1500
  msf2.ColWidth(3) = 1700
  msf2.ColWidth(4) = 1200
  msf2.ColWidth(5) = 1200
  msf2.ColWidth(6) = 1200
  msf2.ColWidth(7) = 3000
  msf2.ColWidth(8) = 1000
  msf2.ColWidth(9) = 500
  msf2.TextMatrix(0, 0) = "Fecha"
  msf2.TextMatrix(0, 2) = "Op."
  msf2.TextMatrix(0, 3) = "Nro.Comprobante"
  If Option4 = True Then
   msf2.TextMatrix(0, 4) = "Debe($)"
   msf2.TextMatrix(0, 5) = "Haber($)"
   msf2.TextMatrix(0, 6) = "Saldo($)"
  Else
   msf2.TextMatrix(0, 4) = "Debe(U$s)"
   msf2.TextMatrix(0, 5) = "Haber(U$s)"
   msf2.TextMatrix(0, 6) = "Saldo(U$s)"
  End If

  msf2.TextMatrix(0, 7) = "Obs."
  msf2.TextMatrix(0, 8) = "Num.Int."
  msf2.TextMatrix(0, 9) = " "
  For i = 0 To 3
    msf2.ColAlignment(i) = 1
  Next i
  For i = 4 To 6
    msf2.ColAlignment(i) = 9
  Next i
  For i = 7 To 8
    msf2.ColAlignment(i) = 1
  Next i
...
Else
  ...
  msf2.ColWidth(5) = 1200
  msf2.ColWidth(6) = 1200
  msf2.ColWidth(7) = 1200
  msf2.ColWidth(8) = 2000
  msf2.ColWidth(9) = 1000
  msf2.ColWidth(10) = 500
  msf2.TextMatrix(0, 0) = "Fecha"
  msf2.TextMatrix(0, 2) = "Op."
  msf2.TextMatrix(0, 3) = "Nro.Comprobante"
  msf2.TextMatrix(0, 4) = "Debe"
  ...
  If Option4 = True Then
   ...
   msf2.TextMatrix(0, 5) = "Haber($)"
   msf2.TextMatrix(0, 6) = "Saldo($)"
   msf2.TextMatrix(0, 7) = "Saldo(U$s)"
  Else
   msf2.TextMatrix(0, 4) = "Debe(U$s)"
   msf2.TextMatrix(0, 5) = "Haber(U$s)"
   msf2.TextMatrix(0, 6) = "Saldo(U$s)"
   msf2.TextMatrix(0, 7) = "Saldo($)"
  End If
  msf2.TextMatrix(0, 8) = "Obs."
  msf2.TextMatrix(0, 9) = "Num.Int."
  msf2.TextMatrix(0, 10) = " "
 For i = 0 To 3
    msf2.ColAlignment(i) = 1
  Next i
  For i = 4 To 7
...
If cl_cli.id > 0 Then
  If cl_cli.idproveedor > 1 Then
      c_prov2.ListIndex = buscaindice(c_prov2, cl_cli.idproveedor)
  Else
      c_prov2.ListIndex = 0
  End If
End If
...
Sub carga2()
  ...
  t2 = 0
  If t_fecha <> "" Then
     q = "select * from a5 where [id_proveedor] = " & c_prov2.ItemData(c_prov2.ListIndex) & " and [ctacte] <> 'N' " & " and [contado] = " & "'N'"
     If Option1 = True Then
        q = q & " and datevalue([fecha]) < datevalue('" & t_fecha & "')"
     Else
        q = q & " and datevalue([fecha_vto]) < datevalue('" & t_fecha & "')"
  ...
  saldoanterioro = sao
  If Check1 = 0 Then
    msf2.AddItem t_fecha & Chr(9) & "" & Chr(9) & "Saldo Ant." & Chr(9) & "" & Chr(9) & Format$(da, "######0.00") & Chr(9) & Format$(ha, "######0.00") & Chr(9) & Format$(sa, "######0.00")
  Else
    msf2.AddItem t_fecha & Chr(9) & "" & Chr(9) & "Saldo Ant." & Chr(9) & "" & Chr(9) & Format$(da, "######0.00") & Chr(9) & Format$(ha, "######0.00") & Chr(9) & Format$(sa, "######0.00") & Chr(9) & Format$(sao, "######0.00")
  End If
  q = "select * from a5, g2 where [id_proveedor] = " & c_prov2.ItemData(c_prov2.ListIndex) & " and a5.[ctacte] <> 'N' and [contado] = 'N' and [id_tipocomp] = [id_tipo_comp]"
  If t_fecha <> "" Then
    If Option1 = True Then
       q = q & " and datevalue([fecha]) >= datevalue('" & t_fecha & "')"
    Else
  ...
  While Not rs.EOF
    ...
    End If
     ...
     o = rs("obs")
     If Check1 = 0 Then
       msf2.AddItem F & Chr(9) & CTC & Chr(9) & tc & Chr(9) & nc & Chr(9) & d & Chr(9) & h & Chr(9) & s & Chr(9) & o & Chr(9) & ni
     Else
        msf2.AddItem F & Chr(9) & CTC & Chr(9) & tc & Chr(9) & nc & Chr(9) & d & Chr(9) & h & Chr(9) & s & Chr(9) & sao & Chr(9) & o & Chr(9) & ni
     End If
    rs.MoveNext
  Wend
...
Call carga_proveedores(c_prov2)
c_prov2.AddItem "<No es Proveedor>", 0
c_prov2.ListIndex = 0
...
Me.StatusBar1.Panels.Item(2) = "[F7] Imprime - [ENTER] Visualiza Comprobante  "
If msf1.Rows > 1 Then
...
If KeyCode = vbKeyF7 Then
  ...
  If para.id_grupo_modulo_actual >= 4 Then
    ...
    Else
      ...
      Next i
     End If
     Call imprimegrid(msf1, c(), "ESTADO DE CUENTA CLIENTES", "", "Cliente: " & c_prov, "Periodo: " & t_fecha & "  " & t_fecha2, 85, 7, True, False)
...
If KeyCode = vbKeyF7 Then
  ...
  If para.id_grupo_modulo_actual >= 4 Then
    ...
    Else
      ...
      Next i
     End If
     Call imprimegrid(msf2, c(), "ESTADO DE CUENTA PROVEEDORES", "", "Proveedor: " & c_prov2, "Periodo: " & t_fecha & "  " & t_fecha2, 85, 7, True, False)
...
If KeyAscii = 13 Then
  If msf2.Row > 0 Then
    Load cc_detalle
    cc_detalle.T_IDPROV = c_prov2.ItemData(c_prov2.ListIndex)
    cc_detalle.t_prov = c_prov2
    cc_detalle.t_sucursal = Mid$(msf2.TextMatrix(msf2.Row, 3), 3, 4)
    cc_detalle.t_letra = Mid$(msf2.TextMatrix(msf2.Row, 3), 1, 1)
    cc_detalle.t_numcomp = Mid$(msf2.TextMatrix(msf2.Row, 3), 8, 8)
    cc_detalle.t_tipocomp = msf2.TextMatrix(msf2.Row, 1)
    If Check1 = 0 Then
      cc_detalle.t_numint = msf2.TextMatrix(msf2.Row, 8)
    Else
       cc_detalle.t_numint = msf2.TextMatrix(msf2.Row, 9)
    End If
    cc_detalle.Show
  End If
End If
