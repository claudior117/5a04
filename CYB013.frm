...
Begin VB.Form cyb_depositoS 
   ...
   End
End
Attribute VB_Name = "cyb_depositoS"
...
Sub graba()
         ...
         rs.Open q, cn1
         ctabanco = rs("id_CUENTA_dep_dif")
         Set rs = Nothing
'         Set rs = New ADODB.Recordset
'         q = "select * from c_01 where [id_cuenta] = " & ctabanco
'         rs.Open q, cn1
'         If Not rs.EOF And Not rs.BOF Then
'           dctabanco = rs("descripcion")
'         Else
'           dctabanco = "Cuenta Inexistente"
'         End If
'         Set rs = Nothing
      ...
      If Val(msf2.TextMatrix(i, 0)) >= 50 Then 'ch. propios
        ...
        QUERY = QUERY & " VALUES (" & Val(msf2.TextMatrix(i, 0)) & ", '" & t_fecha & "', " & Val(msf2.TextMatrix(i, 6)) & ", 1, '" & msf2.TextMatrix(i, 7) & "', 'D', 'N', '" & t_fecha & "', " & Val(msf2.TextMatrix(i, 2)) & ", '" & Left$(Detalle, 39) & " ', 'B', " & numintb & ", 1)"
        cn1.Execute QUERY
      ...
      If Not rs.BOF And Not rs.EOF Then
        If rs("caja") = "S" Then
          ...
          If Val(msf2.TextMatrix(i, 0)) = 3 Then 'ch. terc.
           ...
           QUERY = QUERY & " VALUES (" & Val(msf2.TextMatrix(i, 9)) & ", " & ctabanco & ", '" & Left$(Detalle & " Nro." & t_numcomp, 49) & " ', " & Val(msf2.TextMatrix(i, 6)) & ", 'H', '" & t_fecha & "', " & numintb & ", 'B', 'Dep. " & Format$(numintb, "00000000") & "', " & Val(msf2.TextMatrix(i, 0)) & ", " & nicht & ", " & para.id_usuario & ")"
           cn1.Execute QUERY
        End If
      End If
      Set rs = Nothing
   ...
   If Generaasientosauto Then
      ...
      Set rs = New ADODB.Recordset
      q = "select * from cyb_06 where [id_tipomov] = 50"
      rs.Open q, cn1
      If Not rs.EOF And Not rs.BOF Then
      ...
      If contab <> "N" Then
         ...
         QUERY = QUERY & " VALUES (" & numintcgr & " ,'" & t_fecha & "', '[Depositos] N.I." & Format$(numintb, "00000000") & "', 'B', " & numintb & ", " & Val(t_ingresado) & ", " & Val(t_ingresado) & ", " & para.id_usuario & ", '" & Left$(Detalle & " Dep.Nro." & t_numcomp, 50) & "')"
         cn1.Execute QUERY
      
         'cuenta madre bancos
         QUERY = "INSERT INTO c_03([num_interno], [renglon], [id_cuenta], [ubicacion], [importe], [descripcion])"
         QUERY = QUERY & " VALUES (" & numintcgr & ", 1, " & ctabanco & ", '" & u1 & "', " & Val(t_ingresado) & ", 'Dep.N.I" & Format$(numintb, "00000000") & "')"
         cn1.Execute QUERY
...
ERRORGRABA:
  cn1.RollbackTrans
  MsgBox ("Error de Actualizacion. Verifique los datos o sus permisos. Proc:Depositos  Funcion:Graba")
...
Me.StatusBar1.Panels.Item(2) = "[F1] Ch.Terc. - [F2] Ch.Propios - [F3] Otros  - [F9] Continua - [F5] Elimina Renglon "
If msf2.Rows > 0 Then
