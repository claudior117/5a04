...
Begin VB.Form gen_padronib 
   ...
   End
End
Attribute VB_Name = "gen_padronib"
...
If verifica Then
 J = MsgBox("Confirma Actualizacion del Padron. ", 4)
 If J = 6 Then
  'borrar bd.actual
  'If abrirconexionib Then
   ...
   q = "select * from i2"
   rs.Open q, cnib, adOpenDynamic, adLockOptimistic
   Label1 = "Actualizando padron Retenciones de IB.."
   Label1.Refresh
   ...
   While Not EOF(1)
    ...
    rs("tasa_perc") = 0
    rs("tasa_ret") = Val(Mid$(l, 48, 1) & "." & Mid$(l, 50, 2))
...
Line Input #1, l
If Len(l) <> 55 Then
   MsgBox ("El Archivo DE RETENCIONES no parece tener el formato del padron. Verifiquelo")
   verifica = False
   Exit Function
End If
Close #1
...
Exit Function
errorib:
  MsgBox ("Archivo de RETENCIONES Inexistente o Invalido")
  verifica = False
  Close #1
  Exit Function
