...
Begin VB.Form vta_vencimientos 
   ...
   End
End
Attribute VB_Name = "vta_vencimientos"
...
espere.Show
  ...
  espere.Refresh
  Call armagrid
  q = "select * from vta_02, vta_06 where vta_02.[id_tipocomp] = vta_06.[id_tipocomp]  and vta_02.[sucursal_ingreso] = vta_06.[sucursal] and vta_02.[venta] = 'S' and [saldo_impago02] > 0"
  c = " and "
 ...
 If Option1 = True Then
    q = q & " order by [cliente02], [fecha_vto]"
 Else
    q = q & " order by [fecha_vto], [cliente02]"
 End If
  ...
  While Not rs.EOF
     F = rs("fecha_vto")
     nc = rs("abreviatura") & " " & rs("letra") & " " & Format$(rs("vta_02.sucursal"), "0000") & "-" & Format$(rs("num_comp"), "00000000")
     ...
     Else
       m = "D"
     End If
     If m = rs("vta_02.moneda") Then
       d = Format$(rs("total"), "######0.00")
       t = t + Val(d)
      Else
       If m = "P" Then
         'informe en p y comp en dolares
         d = Format$(rs("total") * rs("cotizacion_dolar"), "######0.00")
         t = t + Val(d)
       Else
    ...
    If m = "P" Then
       si = Format$(rs("saldo_impago02"), "######0.00")
       tsi = tsi + Val(si)
    Else
       si = Format$(rs("saldo_impago02") / rs("cotizacion_dolar"), "######0.00")
       tsi = tsi + Val(si)
    End If
...
Me.StatusBar1.Panels.item(2) = "[F7] Imprime - [F11] Excel - [ENTER] Visualiza comp."
End Sub
...
If KeyAscii = 13 Then
  If msf1.Row > 0 Then
    Load vta_cc_detalle
    vta_cc_detalle.t_numint = msf1.TextMatrix(msf1.Row, 5)
    vta_cc_detalle.Show
  End If
End If
